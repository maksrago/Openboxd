...
movies.get("/",queries.asyncMiddleware,(req,res)=>{
	...
	.then(body => {
	    var results = JSON.parse(body).results.slice(0,12);
	    var popular = [];
...
movies.post("/search",(req,res)=>{
	...
	.then(body => {
	    var search = JSON.parse(body).results.slice(0,25);
	    var search_results = []
	    ...
	    return res.render("search",{"req":req,"search_term":req.body.title,"search_results":search_results} )
...
movies.get("/:id",queries.reviewMiddleware,(req,res)=>{
	...
	.then(body => {
	    ...
	    return res.render("moviePage",{"movieInfo":movieInfo,"movie_reviews":res.locals.movieIdReviews,"req":req});
	})
	.catch(error => {
	    res.sendStatus(404); return;
	});
